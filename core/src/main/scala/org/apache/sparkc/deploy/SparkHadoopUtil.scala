...
package org.apache.sparkc.deploy
 ...
 */
private[sparkc] class SparkHadoopUtil {
   ...
   */
  private[sparkc] def addDelegationTokens(tokens: Array[Byte], sparkConf: SparkConf): Unit = {
  ...
  private[sparkc] def getSuffixForCredentialsPath(credentialsPath: Path): Int = {
   ...
   */
  private[sparkc] def dumpTokens(credentials: Credentials): Iterable[String] = {
    if (credentials != null) {
   ...
   */
  private[sparkc] def tokenToString(token: Token[_ <: TokenIdentifier]): String = {
...
private[sparkc] object SparkHadoopUtil {
   ...
   */
  private[sparkc] val UPDATE_INPUT_METRICS_INTERVAL_RECORDS = 1000
   ...
   */
  private[sparkc] val SPARK_HADOOP_CONF_FILE = "__spark_hadoop_conf__.xml"
   ...
   */
  private[sparkc] val SOURCE_SPARK = "Set by Spark"
   ...
   */
  private[sparkc] def newConfiguration(conf: SparkConf): Configuration = {
